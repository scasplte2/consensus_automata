...
# plt.ylim([0.0, 0.16])
# plt.tight_layout()
# matplotlib.pyplot.savefig('frequency_vs_delay.pdf')
...
plt.xlabel("Block time interval (slot)$")
plt.ylabel("Global block production probability")
plt.xlim([0, 100])
plt.ylim([0.0, 0.08])

plt.tight_layout()
matplotlib.pyplot.savefig('Block_time_interval.png')

# Export Praos data to separate CSV files
# np.savetxt("block_time_experiment_keys.csv", np.fromiter(histogram_data_2.keys(), dtype=float), delimiter=",")
# np.savetxt("block_time_experiment_values.csv", values2, delimiter=",")
